...
async function JobsTable({ q, status, saved }: { q: string; status: "ACTIVE" | "EXPIRED" | "ALL"; saved?: string }) {
  ...
  return (
    <>
      {saved && (
        <p role="status" className="border-line-strong bg-card text-fg mb-4 rounded-lg border px-4 py-2 text-sm">
          Saved.{" "}
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
        <h1 className="text-xl font-semibold">
          Jobs <span className="text-muted text-sm font-normal tabular-nums">({total})</span>
        </h1>
        <Form action="/admin" className="flex gap-2">
          ...
          <button className="border-line-strong hover:bg-card h-10 rounded-lg border px-4 text-sm">Filter</button>
        </Form>
      </div>
            ...
            {jobs.map((job) => (
              <tr key={job.id} className="bg-card align-top">
                <td className="px-3 py-2.5">
                  <Link href={`/admin/jobs/${job.id}`} className="text-fg font-medium hover:underline">
                    {job.title}
                  </Link>
                ...
                <td className="text-muted px-3 py-2.5 tabular-nums">
                  {formatSalaryBand(job.salaryMin, job.salaryMax, job.currency) ?? (
                    <span className="text-subtle">not disclosed</span>
                  )}
                  {job.source !== "MANUAL" && (
                    <div className="text-subtle text-[11px]">{job.source.toLowerCase()} feed</div>
                  )}
                </td>
                <td className="px-3 py-2.5">
                  <span className={job.status === "ACTIVE" ? "text-ok" : "text-subtle"}>
                    {job.status.toLowerCase()}
                  </span>
                </td>
                <td
                  className={`px-3 py-2.5 ${job.status === "ACTIVE" && job.lastVerifiedAt < staleBefore ? "text-danger" : "text-muted"}`}
                >
                  {formatPostedAgo(job.lastVerifiedAt)}
                </td>
                <td className="text-muted px-3 py-2.5 text-right tabular-nums">{job._count.applyClicks}</td>
                <td className="px-3 py-2.5">
                  <div className="flex justify-end gap-3 text-xs">
                    ...
                    <form action={setJobStatus}>
                      ...
                      <button className="text-muted hover:text-fg">
                        {job.status === "ACTIVE" ? "Take down" : "Re-open"}
                      </button>
                    </form>
      ...
      {total > jobs.length && (
        <p className="text-subtle mt-3 text-xs">Showing the newest {jobs.length}. Narrow the search to find others.</p>
      )}
