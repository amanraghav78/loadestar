import { z } from "zod";
import { Currency, Discipline, Level, RemotePolicy } from "@/lib/generated/prisma/enums";
...
export const searchParamsSchema = z.object({
  ...
  minSalary: lenient(z.coerce.number().int().min(0).max(2_000_000)),
...
export const jobInputSchema = z
  .object({
    ...
    salaryMin: z.coerce.number().int().min(1).max(2_000_000),
    salaryMax: z.coerce.number().int().min(1).max(2_000_000),
